...
class BertModel(nn.Module):
    def __init__(self,device):
        ...
        self.is_bilstm = False # 使用bilstm
        self.bert = ClassifyClass.from_pretrained(bert_path_or_name)
        self.device = device
        self.dropout = nn.Dropout(0.1)
        if self.is_bilstm:
            if "large" in MODEL:
                self.classifier = nn.Linear(1024 * 2, num_labels)
                self.bilstm = nn.Sequential(
                    nn.LSTM(input_size=1024, hidden_size=1024, num_layers=1, bidirectional=True, batch_first=True),
                )

            else:
                self.classifier = nn.Linear(768* 2, num_labels)
                self.bilstm = nn.Sequential(
                    nn.LSTM(input_size=768, hidden_size=768, num_layers=1, bidirectional=True, batch_first=True),
                )
        else:
            if "large" in MODEL:

...
class ACDataLoader(object):
    def __init__(self, data, batch_size):
        self.uids = np.array(data['uids'])
        self.attribute_text = np.array(data['attribute_text'], dtype=object)
        self.attribute_ids = np.array(data['attribute_ids'], dtype=object)
        self.num_users = len(self.uids)
    ...
    def get_batch(self):
        ...
        batch_uids = self.uids[batch_idx].tolist()
        batch_attribute_text = self.attribute_text[batch_idx].tolist()
        batch_attribute_ids = self.attribute_ids[batch_idx].tolist()
        self._has_next = self._has_next and end_idx < self.num_users
        self._start_idx = end_idx
        batch_out = {'uids': batch_uids, 'attribute_text': batch_attribute_text, 'attribute_ids': batch_attribute_ids}
        return batch_out


def train(args):
    env = BatchKGEnvironment(args)
    with open(TMP_DIR[args.dataset] + 'train.pkl', 'rb') as f:
        train = pickle.load(f)
    uids = [i[0][0] for i in train]
    attri_text = [i[4] for i in train]
    attri_id = [i[3] for i in train]
    data = {'uids': uids, 'attribute_text': attri_text, 'attribute_ids': attri_id}
    dataloader = ACDataLoader(data, args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            batch_data = dataloader.get_batch()
            # Start batch episodes
            batch_state = env.reset(batch_data['attribute_ids'], batch_data['attribute_text'],
                                    batch_data['uids'])  # numpy array of [bs, state_dim]
            done = False
            while not done:
    ...
    policy_file = '{}/policy_model_epoch_{}.ckpt'.format(args.log_dir, args.epochs)
